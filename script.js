class App {
  ...
  renderApp() {
    ...
    const links = user.role === 'professor'
      ? [{ label:'Turmas', anchor:'turmas', icon:this.iconClass() },
         ...
         { label:'Administração', anchor:'admin', icon:this.iconSettings() },
         { label:'Configurações', anchor:'config', icon:this.iconUser() }]
    ...
    const navigate = () => {
      ...
      switch(hash) {
        ...
        case '#config': this.renderConfig(main, user); break;
        default:main.innerHTML='<p>Página não encontrada</p>';
      }
    };
  ...
  // Métodos auxiliares como renderTurmas, renderApostilas, renderAvaliacoes, renderAdmin, renderConfig,
  // e métodos para modal, ícones SVG etc. seguem modelo implementado anteriormente e são todos funcionais.
